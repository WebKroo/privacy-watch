...
- The shield opens a compact panel with status, Pause/Resume, Activity & Settings, and Quit. File paths and CSV/folder buttons are in Activity & Settings. Sensor switches live in Settings. While its window is open, Privacy Watch has a normal macOS application menu and Dock icon. **Privacy Watch → Quit Privacy Watch** or Command-Q stops logging and quits. Closing only the window leaves logging running.
